...
namespace Tga
{
	...
	public:
		...
		static void DrawQueued(RenderTarget* aRenderTarget = DX11::BackBuffer);
		...
		static bool DrawCanvasSlider(UISlider& slider, const GeneralUIProperties& props, const CanvasObjectDefinition& canvas, CanvasDrawParameters& drawParameters);
		...
		static Texture* GetTexture(StringId path, TextureSrgbMode srgbMode);


	private:
		static std::unordered_map<StringId, Texture*> myTextureCache;
		...
		static void QueueSprite(SpriteSharedData aSharedData, Sprite2DInstanceData aInstance);
	};
}
