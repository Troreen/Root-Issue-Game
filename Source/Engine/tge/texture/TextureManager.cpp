...
bool IsTarga(const std::string& aPath)
{
	size_t index = aPath.find_last_of(".");
	std::string substr = aPath.substr(index +1);

	if (lstrcmpA(substr.c_str(), "tga") == 0)
	{
		return true;
	}
	else
...
Texture* TextureManager::GetTexture(const char* aTexturePath, TextureSrgbMode aSrgbMode, bool aForceReload)
{
	...
	std::string stringId = aTexturePath;
...
Texture* TextureManager::TryGetTexture(const char* aTexturePath, TextureSrgbMode aSrgbMode, bool aForceReload)
{
	...
	const std::string asset_path = Settings::ResolveAssetPath(aTexturePath);

	if (asset_path.empty())
		return nullptr;

	if (!loadedTexture)
		Engine::GetInstance()->GetFileWatcher()->WatchFileChange(asset_path, std::bind(&Tga::TextureManager::OnTextureChanged, this, stringId));

	const std::wstring asset_path_w = string_cast<std::wstring>(asset_path);
...
void TextureManager::OnTextureChanged(std::string aFile)
{
	...
	INFO_PRINT("%s%s", "Texture changed: ", aFile.c_str());
...
Vector2f TextureManager::GetTextureSize( ID3D11ShaderResourceView* aResourceView, bool aNormalize)
{
	...
	Vector2f windowSize;
	windowSize.x = static_cast<float>(Engine::GetInstance()->GetRenderSize().y);
	windowSize.y = static_cast<float>(Engine::GetInstance()->GetRenderSize().y);
