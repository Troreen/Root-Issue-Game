...
Font TextService::GetOrLoad(std::string aFontPathAndName, FontSize aFontSize, unsigned char aBorderSize)
{
	if (aFontSize == -1)
	{
		return { nullptr };
	}
	std::string resolvedPath = Tga::Settings::ResolveAssetPath(aFontPathAndName);
	...
	FT_Face face;
	FT_Error error = FT_New_Face(myLibrary, resolvedPath.c_str(), 0, &face);
	if (error != 0)
	{
		return { nullptr };
	}
