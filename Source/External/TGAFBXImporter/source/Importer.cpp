...
namespace TGA
{
	namespace FBX
	{
		...
		void Importer::SetShouldBakeSkeletonRootTransforms(bool shouldBake)
		{
			Internals::ourShouldBakeSkeletonRootTransforms = shouldBake;
		}
			...
			if (fbxImporter->Import(fbxScene))
			{
				...
				for(FbxNode* meshNode : sceneMeshNodes)
				{
					...
					if(numUvLayers != 0)
					{
						...
						{
							fbxMesh->GenerateTangentsData(0, false, false);
						}
					}
				}				
		...
		bool Importer::LoadAnimationA(const std::string& someFilePath, Animation& outAnimation)
		{
			...
			if (fbxImporter->Import(fbxScene))
			{
				...
				if (!fbxHasBones)
				{
					...
					locLastError = "The file[" + someFilePath + "] has no skeleton!";
					return false;
				}
				...
				FbxNode* skeletonRoot = fbxScene->FindNodeByName(FbxString(rootBone->NamespaceName.c_str()));
				FbxAMatrix skeletonParentTransform = skeletonRoot->GetParent()->EvaluateGlobalTransform();
				FbxAMatrix skeletonParentTransformInv = skeletonParentTransform.Inverse();
				...
				if(animStackCount == 0)
				{
					...
					locLastError = "The file [" + someFilePath + "] has no animation stack!";
					return false;
				}
				...
				for (FbxLongLong t = startFrame; t <= endFrame; t++)
				{
					...
					for (Skeleton::Bone& Bone : mdlSkeleton.Bones)
					{
						...
						{
							if (Internals::ourShouldBakeSkeletonRootTransforms)
							{
								localFrameTransform = skeletonParentTransform * jointNode->EvaluateLocalTransform(currentTime) * skeletonParentTransformInv;
								globalFrameTransform = skeletonParentTransform * jointNode->EvaluateGlobalTransform(currentTime) * skeletonParentTransformInv;
