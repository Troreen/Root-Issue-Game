...
namespace TGA
{
	namespace FBX
	{
		...
		bool Internals::ourShouldBakeSkeletonRootTransforms = true;
		                              ...
		                              std::vector<Mesh::Element>& outMeshes, bool bMergeDuplicateVertices)
		{
			...
			if(aSkeleton.GetRoot())
			{
				...
				const Skeleton::Bone* rootBone = aSkeleton.GetRoot();
				FbxNode* skeletonRoot = aScene->FindNodeByName(FbxString(rootBone->NamespaceName.c_str()));
				...
				for (int deformIdx = 0; deformIdx < fbxMesh->GetDeformerCount(); deformIdx++)
				{
					...
					for (int clusterIdx = 0; clusterIdx < fbxSkin->GetClusterCount(); clusterIdx++)
					{
						...
						fbxsdk::FbxAMatrix bindPoseInverseTransform;
						if (Internals::ourShouldBakeSkeletonRootTransforms)
						{
							bindPoseInverseTransform = skeletonParentTransform * linkTransform.Inverse() * meshBindTransform * rootTransform;
						}
						else
						{
							bindPoseInverseTransform = linkTransform.Inverse() * meshBindTransform * rootTransform;
						}
