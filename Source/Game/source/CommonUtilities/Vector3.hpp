...
namespace CommonUtilities
{
	...
	public:
		...
		//Creates a vector from a TGA Vector3 for engine boundary compatibility
		Vector3(const Tga::Vector3<T>& aVector);

		//Copy constructor (compiler generated)
		Vector3(const Vector3<T>& aVector) = default;

		//Assignment from a TGA Vector3 for engine boundary compatibility
		Vector3<T>& operator=(const Tga::Vector3<T>& aVector);
	...
	template <typename T>
	inline Vector3<T>::Vector3(const Tga::Vector3<T>& aVector)
		: x(aVector.x)
		, y(aVector.y)
		, z(aVector.z)
	{
	...
	template <typename T>
	inline Vector3<T>& Vector3<T>::operator=(const Tga::Vector3<T>& aVector)
	{
		x = aVector.x;
		y = aVector.y;
		z = aVector.z;
