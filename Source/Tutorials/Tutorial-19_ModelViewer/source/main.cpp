...
struct LightData
{
    ...
    std::wstring cubeMap = string_cast<std::wstring>(Settings::ResolveGameAssetPath("cubemap/Gdansk_shipyard_buildings_cubemap.dds"));
};
...
void CreatePreviewForAsset(std::string filePath)
{
...
bool IsFBX(std::string aPath) //Return true if the filepath ends with .fbx (this is for the drag-and-drop model viewer)
{
    if (aPath.substr(aPath.find_last_of(".") + 1) == "fbx")
    {
        return true;
    }
    else
    ...
    bool& isRightMouseTrapped)
{
    ...
    {
        camMovement += camTransform.GetUp() * mouseDelta.Y;
        camMovement -= camTransform.GetRight() * mouseDelta.X;
        camera->GetTransform().Translate(camMovement);
    }
    else if (isLeftMouseTrapped && isAltDown)
    {
        ...
        cameraRotation.X += mouseDelta.Y * camRotSpeed;
        cameraRotation.Y += mouseDelta.X * camRotSpeed;
        camera->GetTransform().SetRotation(cameraRotation);
        camera->GetTransform().SetPosition((targetPoint)+(camera->GetTransform().GetForward() * -relationCamToTarget));

    }
    else if (isRightMouseTrapped && isAltDown)
    {
        camMovement += camTransform.GetForward() * mouseDelta.Y;
        camera->GetTransform().Translate(camMovement);
    }
    ...
    {
        camera->GetTransform().SetPosition(GlobalRenderData.targetPosition - (camera->GetTransform().GetForward() * 200.0f));
...
void Go(void)
{
    ...
    GlobalLightData.cubeMap = string_cast<std::wstring>(Settings::ResolveGameAssetPath("cubemap/Gdansk_shipyard_buildings_cubemap.dds"));
	...
	cfg.myWinProcCallback = [](HWND, UINT message, WPARAM wParam, LPARAM lParam)
	{
		...
		case WM_DROPFILES:
		{
			...
			UINT num_paths = DragQueryFileA(hDropInfo, 0xFFFFFFFF, 0, 512);

			char* filename = nullptr;
			UINT max_filename_len = 0;

			for (UINT i = 0; i < num_paths; ++i) {
				UINT filename_len = DragQueryFileA(hDropInfo, i, nullptr, 512) + 1;
				if (filename_len > max_filename_len) {
					max_filename_len = filename_len;
					char* tmp = (char*)realloc(filename, max_filename_len * sizeof(*filename));
					if (tmp != nullptr) {
						filename = tmp;
					}
				}
				DragQueryFileA(hDropInfo, i, filename, filename_len);
			}
            //GlobalRenderData.modelName = filename;
			if (filename && IsFBX(filename))
    ...
    cfg.myApplicationName = L"TGA 2D Tutorial 19";
    cfg.myActivateDebugSystems = Tga::DebugFeature::None;//Tga::DebugFeature::Fps |
...

    {
        ...
        GlobalRenderData.myPlane = std::make_shared<ModelInstance>(ModelFactory::GetInstance().GetModelInstance("Plane"));
        GlobalRenderData.myPlane->GetTransform().Scale({ 10 });
        static const char* currentCubeMap = GlobalLightData.s_cubemaps[0].c_str();
        auto aLight = AmbientLight(
            Color{ 0.0f, 0.0f, 0.0f },
            AmbientLightType::Custom,
            Engine::GetInstance()->GetTextureManager().GetTexture(currentCubeMap, TextureSrgbMode::None)
        );
        ...
        auto dLight = DirectionalLight(
            Matrix4x4f::CreateFromRotation({ GlobalLightData.lightDirections[0], GlobalLightData.lightDirections[1], GlobalLightData.lightDirections[2] }),
            Color{ 1, 1, 1 }
        );
        ...
        Vector3f cameraRotation = Rotator(45, 180, 0);
            ...
            if (ImGui::Begin("Light and Rendering", &isOpen, 0))
            {
                ...
                if (ImGui::BeginCombo("CubeMaps", currentCubeMap))
                {
                    for (int n = 0; n < GlobalLightData.s_cubemaps.size(); n++)
                    {
                        ...
                        {
                            AmbientLight newALight = AmbientLight(
                                Color{ 0.0f, 0.0f, 0.0f },
                                AmbientLightType::Custom,
                                Engine::GetInstance()->GetTextureManager().GetTexture(GlobalLightData.s_cubemaps[n].c_str(), TextureSrgbMode::None)
                            );
...
#endif
#pragma endregion
            GlobalRenderData.myDirectionalLight->transform.SetRotation(Rotator(lightDirections[0], lightDirections[1], lightDirections[2]));
            GlobalRenderData.myDirectionalLight->color = Tga::Color(rgb[0], rgb[1], rgb[2]);
...
#pragma region Camera
            ...
            //    const Vector2f mouseDelta = myInputManager.GetMouseDelta();
                HandleInput(myInputManager, cameraRotation, camera, camRotSpeed, isScrollMouseTrapped, isAltDown, isLeftMouseTrapped, isRightMouseTrapped);
